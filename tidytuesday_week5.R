...
hardness = c('shedding_level','coat_grooming_frequency','drooling_level','adaptability_level','trainability_level',
...
for (i in c(1:nr)) {
  ...
  if(breed_traits[i,2] %in% hardness){breed_traits[i,2] = 'hardness'}
...
breed_traits = breed_traits %>% filter(hardness > 0, agreeability > 0) #delete non-positive scores

#Create the chart
chart_dogs_score = 
  ggplot(breed_traits, aes(x = hardness, y = agreeability)) +
